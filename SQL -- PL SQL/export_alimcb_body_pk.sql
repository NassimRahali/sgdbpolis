...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Package Body ALIMCB_PKG
--------------------------------------------------------

  CREATE OR REPLACE PACKAGE BODY "NASSIM"."ALIMCB_PKG" AS
  ...
  BEGIN
    ...
    INSERT INTO MOVIES VALUES (movie.movie_id, movie.title,
                               movie.overview, movie.release_date,
                               vote_avg, vote_cnt, 
                               cert_id, movie.production_country,
                               runt, movie.copies);
  ...
  BEGIN
    FOR i IN 1 .. companies.COUNT LOOP
      SELECT COUNT(*) INTO company_found FROM COMPANIES WHERE id = companies(i).id;
  ...
  BEGIN
    ...
    IF certif_found = 0 THEN
      INSERT INTO CERTIFICATIONS(id, name, description) VALUES(SEQUENCE_CERTIF.NEXTVAL, certif.name, certif.description);
